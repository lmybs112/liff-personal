;(function () {
  function Product_Recommendation(config = {}) {
    ...
    const defaultConfig = {
      ...
      ctype_val: ['underwear'], //  JSON.stringify(["underwear"])
      bid: {
        HV: '163',
        WV: '50',
        CC: '97.5_97.5',
      ...
      breakpoints: {
        ...
        0: {
          ...
          speed: 750,
          resistanceRatio: 0,
          loopFillGroupWithBlank: true // 確保手機版也能正確填充
        }
      }
    }
    ...
    function loadEmbeddedScript($) {
      ;(function ($) {
        ...
        $(function () {
          ...
          $(window).resize(function () {
            ...
            if (arrowPosition === 'none') {
              ...
              if (arrowPosition === 'center' && newContainerWidth >= 768) {
                ...
                $(`#${containerId} .title-navigation`).css('display', 'inline-flex')
              }
            }
            ...
            $(show_up_position_before)
              .css(
                'padding',
                !!customPadding ? customPadding : newContainerWidth >= 768 ? '32px' : '8px'
              )
          ...
          if (typeof $(show_up_position_before)[0] !== 'undefined') {
            ...
            $(show_up_position_before)
              .css(
                'padding',
                !!customPadding ? customPadding : containerWidth >= 768 ? '32px' : '8px'
              )
        ...
        $(document).on('click', `#${containerId} #swiper-wrapper-corr .embeddedItem`, function () {
          const title = $(this).data('title') // 取得 data-title 屬性
          const link = $(this).data('link') // 取得 data-link 屬性
        ...
        $(document).on('click', `#${containerId} #swiper-wrapper-corr .a-left`, function () {
          // 觸發 Google Analytics 的事件追蹤
          gtag('event', 'corr_click_embedded_item' + test, {
            send_to: GA4Key,
            event_category: 'swiper-wrapper-corr-embedded',
        ...
        $(document).on('click', `#${containerId} #swiper-wrapper-corr .a-right`, function () {
          // 觸發 Google Analytics 的事件追蹤
          gtag('event', 'corr_click_embedded_item' + test, {
            send_to: GA4Key,
            event_category: 'swiper-wrapper-corr-embedded',
        ...
        $(document).on('click', `#${containerId} #swiper-wrapper-basic .embeddedItem`, function () {
          const title = $(this).data('title') // 取得 data-title 屬性
          const link = $(this).data('link') // 取得 data-link 屬性
        ...
        $(document).on('click', `#${containerId} #swiper-wrapper-basic .a-left`, function () {
          // 觸發 Google Analytics 的事件追蹤
          gtag('event', 'bhv_click_embedded_item' + test, {
            send_to: GA4Key,
            event_category: 'swiper-wrapper-basic-embedded',
        ...
        $(document).on('click', `#${containerId} #swiper-wrapper-basic .a-right`, function () {
          // 觸發 Google Analytics 的事件追蹤
          gtag('event', 'bhv_click_embedded_item' + test, {
            send_to: GA4Key,
            event_category: 'swiper-wrapper-basic-embedded',
        ...
        function getEmbeddedAds(ids) {
          ...
          if (!hide_size) {
            ...
            requestData.bid = bid
            //FIXME check key
            // requestData.SIZEAI_ptr = 'bhv,sp_atc,sp_trans'
            // requestData.SP_PID = 'xxx'
          }
          const options = {
            method: 'POST',
            headers: {
            ...
            .then((response) => {
              ...
              if (response['SIZEAI_result']) {
                size_tag = response['SIZEAI_result'].reduce((acc, item) => {
                  // Check if item is an object and has a non-empty ITEM property
                  if (item && item.ITEM && typeof item.ITEM === 'string' && item.ITEM !== '{}') {
                    try {
                      const itemScores = JSON.parse(item.ITEM);
                      // Ensure itemScores is an object and not empty
              ...
              if (jsonData.length > 0) {
                ...
                // 如果沒有商品，隱藏整個容器
                $(`#${containerId} #recommendation-loading`).fadeOut(400)
                ...
                // $(`#${containerId} .embeddedAdContainer`).hide()
              }
            })
            .catch((err) => {
              console.error(err)
        ...
        function updatePopAd(images, corr_bool) {
          ...
          // 如果沒有商品，創建空的商品來確保 Swiper 能正常初始化
          if (images.length === 0) {
            displayImages = [{
              link: '#',
              title: '暫無商品',
    ...
    <a class="embeddedItem swiper-slide" href="${
      ...
      ${img.size_tag ? `<div class="embeddedItem__sizeTag">${img.size_tag}</div>` : ''}
          <img loading="lazy" src="${img.image_link}" alt="${
            img.description
    ...
    `
          ...
          var swiper = new Swiper(`.swiper-basic-${containerId}`, {
            ...
            slidesPerView: 1,
            slidesPerGroup: 1,
            loopFillGroupWithBlank: true, // 啟用空白幻燈片填充
            spaceBetween: 8,
